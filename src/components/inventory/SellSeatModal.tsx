...
export default function SellSeatModal({
  ...
  function handleChange(
    ...
    setForm(prev => {
      ...
      // Auto-adjust price based on buyer type
      if (name === 'sold_to_type') {
        if (value === 'customer')    next.sold_price = batch.retail_price?.toString()
        if (value === 'sub_agent')   next.sold_price = batch.agent_price?.toString()
        if (value === 'agency')      next.sold_price = batch.agency_price?.toString()
        if (value === 'own_booking') next.sold_price = batch.cost_per_seat?.toString()
      }
      return next
    })
  }
...
<div className="p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-3">
            ...
            {/* Add this new block right after the grid above, still inside the is_spot check */}
{form.is_spot === 'true' && (
